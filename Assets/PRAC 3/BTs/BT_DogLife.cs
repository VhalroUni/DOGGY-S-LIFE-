...
[CreateAssetMenu(fileName = "BT_DogLife", menuName = "Behaviour Trees/BT_DogLife", order = 1)]
public class BT_DogLife : BehaviourTree
{
    ...
    public override void OnConstruction()
    {
      ...
      */
        DynamicSelector dogLife = new DynamicSelector();

        dogLife.AddChild(new CONDITION_InstanceNear("1000", "BULLY"), 
            new ACTION_Hide()
        );

        dogLife.AddChild(new CONDITION_IsHungry(),
            new BT_Eat()
        );

        dogLife.AddChild(new CONDITION_InstanceNear("1000", "STICK", "true", "TheStick"),
            new BT_ChaseStick()
        );

        dogLife.AddChild(new CONDITION_AlwaysTrue(),
            new BT_RelaxedWalk()
        );

        root = new RepeatForeverDecorator(dogLife);
